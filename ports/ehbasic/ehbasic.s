...
*														*
*	This is the generic version with I/O and LOAD/SAVE example code for CP/M-68k	*
*
*														*
*************************************************************************************
*														*
...
* in d0. 
    ...
    move.w  #18,d0  ; Get address of Memory Region Table (MRT)
    trap    #3      ; call BIOS
    movea.l d0,a0
    move.l  6(a0), d0    ; TPA len

*	MOVEA.l	#ram_addr,a0		* tell BASIC where RAM starts
*	MOVE.l	#ram_size,d0		* tell BASIC how big RAM is
	movea.l 2(a0), a1
	movea.l a1, a0
*	MOVE.l	d7,d0	          	* tell BASIC how big RAM is

    jmp     LAB_COLD
    
*************************************************************************************
*
* the following code is cp/m specific, change to suit your system
...
RETCHR
	MOVEQ	#1,d0				* get byte form the keyboard
	TRAP	#2    				* BDOS call
...
*************************************************************************************
*
* LOAD routine for cp/m-68k

VEC_LD
	LEA		load_title(pc),a1	* set the LOAD request title string pointer
	BSR		get_fn		        * get the filename from the line or the request

	BEQ		LAB_FCER			* if null do function call error then warm start

    move.w  #128,byte_cnt(a3)   * set byte counter
    
	MOVE	#15,d0			    * open existing file
	LEA		workfcb2(a3),a1
    clr.b   32(a1)              * clear cr
	move.l  a1,d1
	MOVE.l	d1,file_id(a3)		* save the FCB adddress
	TRAP	#2				    * do BDOS function

    cmpi.w  #$ff,d0				* test open result
	beq.s	LOAD_exit		     * if error clear up and exit
...
LOAD_in
	...
	cmp.w  #128,byte_cnt(a3)    * read new sector ?
	bne.s  LOAD_in1
    move.w  #0,byte_cnt(a3)     * reset byte counter
*	
* set DMA address
*
	LEA		secbuf(a3),a1	    * point to sector buffer
	move.l  a1,d1
...
*
	...
	MOVE.l	file_id(a3),d1		* load the FCB adddress
    trap    #2
    cmpi.w  #0,d0
...
LOAD_in1
	LEA		secbuf(a3),a1	    * point to sector buffer
	move.w  byte_cnt(a3),d1
	MOVE.b	(a1,d1),d0			* get byte
	add.w   #1,byte_cnt(a3)     * inc byte counter
...
*   							* vector and tidy up
LOAD_eof
*	MOVEQ		#16,d0			* close file
*	MOVE.l	file_id(a3),d1		* load the FCB adddress
*	TRAP		#2				* do I/O function
...
get_name:
    ...
    move.w	#10,d0              * read console buffer
	LEA		secbuf(a3),a1
    move.l	a1,d1
    move.b  #8,(a1)             * mx=8 8 chars max
    trap	#2
...
*
	LEA	   secbuf(a3),a2
	clr.w  d1
	move.b 1(a2),d1            * fname lenght (nc)
	sub.l  #1,d1
	LEA	   workfcb2(a3),a1
	add.l  #1,a1
	add.l  #2,a2
...
load_title
	dc.b	'LOAD file (*.bas) : ','$'		* LOAD file title string

save_title
	dc.b	'SAVE file (*.bas) : ','$'		* SAVE file title string

open_err:
    dc.b    13,10,'open failed !',13,10,'$'
open_e1:
    dc.b    13,10,'file already exists !',13,10,'$'
...
*
    ...
    cmpi.w  #$ff,d0				* test open result
	beq.s	SAVE_1		         * if error clear up and exit

    move.w	#9,d0               * print string
	LEA		open_e1(pc),a1
    move.l  a1,d1               
    trap    2
	BRA		LAB_FCER			* if error do function call error, warm start
	
SAVE_1:
	MOVE	#22,d0			    * creeate file
	LEA		workfcb2(a3),a1
    clr.b   32(a1)              * clear cr
	move.l  a1,d1
	MOVE.l	d1,file_id(a3)		* save the FCB adddress
    ...
    cmpi.w  #$ff,d0				* test open result
	beq    	SAVE_exit           * if error clear up and exit
...
*
    ...
    beq     SAVE_2
*
* clear sector buffer from start+byte_cnt to end of buffer !
*
    clr.l   d0
    move.w  byte_cnt(a3),d0
	LEA		secbuf(a3),a1	    * point to sector buffer
	adda.l  d0,a1
	move.w  #128,d1
	sub.w  d0,d1
...
SAVE_OUT:
	...
	LEA		secbuf(a3),a1	    * point to sector buffer
	move.w  byte_cnt(a3),d1
	MOVE.b	d0,(a1,d1)			* save byte
	add.w   #1,byte_cnt(a3)     * inc byte counter
	
	cmp.w  #128,byte_cnt(a3)    * write next sector ?
	bne.s  SAVE_O1
    move.w  #0,byte_cnt(a3)     * reset byte counter
*	
* set DMA address
...
*   							* else get string
	...
	MOVEQ		#$3c,d0			* other search character is ","
...
LAB_2967
	...
	MOVEQ		#$2F,d4			* set character to "0"-1
LAB_2968
...
LAB_CN	dc.b	'Can not continue',$00
